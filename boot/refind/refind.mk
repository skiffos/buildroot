...
REFIND_BINARIES_DIR = $(BINARIES_DIR)/efi-part
REFIND_INSTALL_ARGS = \
...
define REFIND_INSTALL_IMAGES_CMDS
	if [ -d $(REFIND_BINARIES_DIR)/EFI/refind ]; then \
		rm -rf $(REFIND_BINARIES_DIR)/EFI/refind; \
	fi
...
define REFIND_IMAGES_RENAME_TO_BOOT
	if [ -d $(REFIND_BINARIES_DIR)/EFI/BOOT ]; then \
		rm -rf $(REFIND_BINARIES_DIR)/EFI/BOOT; \
	fi
